<?php
  ...
  * Information
  * @Author: xares
  * @Date:   21-05-2020 13:05
  * @Filename: Bitrate.php
  ...
  */
  ...
  class Bitrate extends CI_Controller {
    public function index() {
      ...
      $req = request($this->input->post(),['botID','value'],['Podaj id bota!','Podaj jakość dźwięku!']);
      if(!$req['success']) {
      ...
      if(!is_numeric($req['value'])) {
        return $this->output->set_output(printJson(false,'  Jakość można podać tylko w liczbach!'));
      }

      $run = json_decode($this->ts3ab->command("settings/bot/set/{$req['botID']}/audio.bitrate/{$req['value']}"),true);
