<?php
  ...
  * Information
  * @Author: xares
  * @Date:   21-05-2020 13:29
  * @Filename: LongMessageSplitLimit.php
  ...
  * @Last Modified time: 21-05-2020 13:31
  *
  * @Copyright(C) 2020 x-Scripts
  */

  class LongMessageSplitLimit extends CI_Controller {
    public function index() {
      ...
      $req = request($this->input->post(),['botID','value'],['Podaj id bota!','Podaj podział długich wiadomości!']);
      if(!$req['success']) {
      ...
      if(!is_numeric($req['value'])) {
        return $this->output->set_output(printJson(false,'Można podawać tylko liczby!'));
      }

      $run = json_decode($this->ts3ab->command("settings/bot/set/{$req['botID']}/commands.long_message_split_limit/{$req['value']}"),true);
