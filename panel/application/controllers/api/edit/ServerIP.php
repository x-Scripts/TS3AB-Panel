<?php
  ...
  * Information
  * @Author: xares
  * @Date:   21-05-2020 12:08
  * @Filename: ServerIP.php
  ...
  */
  ...
  class ServerIP extends CI_Controller {
    public function index() {
      ...
      $req = request($this->input->post(),['botID','value'],['Podaj id bota!','Podaj ip serwera!']);
      if(!$req['success']) {
      ...
      foreach(json_decode($this->ts3ab->botList(),true) as $index) {
        if($index['Status'] && $index['Name'] == $req['botID']) {
          if($index['Server'] != $req['value']) {
            $this->ts3ab->setBotID($index['Id']);
            $disconnect = json_decode($this->ts3ab->botDisconnect(),true);
            if(isset($disconnect['ErrorMessage'])) {
              return $this->output->set_output(printJson(false,$disconnect['ErrorMessage']));
            }
            $connect = true;
          }
        }
      ...
      $run = json_decode($this->ts3ab->command("settings/bot/set/{$req['botID']}/connect.address/\"{$req['value']}\""),true);
      if(isset($run['ErrorMessage'])) {
        return $this->output->set_output(printJson(false,$run['ErrorMessage']));
      }

      if($connect) {
        $this->ts3ab->botConnect($req['botID']);
      }
