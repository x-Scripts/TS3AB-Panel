<?php
  ...
  * Information
  * @Author: xares
  * @Date:   21-05-2020 14:10
  * @Filename: DeleteAlias.php
  ...
  * @Last Modified time: 21-05-2020 14:16
  *
  * @Copyright(C) 2020 x-Scripts
  */

  class DeleteAlias extends CI_Controller {
    public function index() {
      ...
      $req = request($this->input->post(),['botID','value'],['Podaj id bota!','Podaj jaką komendę usunąć!']);
      if(!$req['success']) {
      ...
      if(empty($req['value'][0]) && empty($req['value'][1])) {
        return $this->output->set_output(printJson(false,'Podaj jaką komendę usunąć!'));
      }
      ...
      foreach(json_decode($this->ts3ab->botList(),true) as $index) {
        if($index['Name'] == $req['botID']) {
          if($index['Status']) {
            ...
            $run = json_decode($this->ts3ab->commandBot("alias/remove/\"".rawurlencode($req['value'])."\""),true);
            $added = false;
          } else {
            return $this->output->set_output(printJson(false,'Najpierw uruchom bota aby usunąć komendę!'));
          }
        }
      }

      if(isset($run['ErrorMessage'])) {
        return $this->output->set_output(printJson(false,$run['ErrorMessage'].' - '.$run['ErrorCode']));
      }
