<?php
  ...
  * Information
  * @Author: xares
  * @Date:   24-05-2020 19:20
  * @Filename: PlaySong.php
  ...
  */
  ...
  class PlaySong extends CI_Controller {
    public function index() {
      ...
      $req = request($this->input->post(),['botID','url'],['Podaj id bota!','Podaj link do utworu!']);
      if(!$req['success']) {
      ...
      foreach($list as $index) {
        if($index['Name'] == $req['botID']) {
          if($index['Status'] == 2) {
            ...
            $player = json_decode($this->ts3ab->play($req['url']),true);
            if(isset($player['ErrorCode'])) {
              return $this->output->set_output(printJson(false,$player['ErrorMessage']));
            }
            return $this->output->set_output(printJson(true,'Uruchomiono utwór!'));
          }
        }
      }
