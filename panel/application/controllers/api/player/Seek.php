  ...
  */
  ...
  class Seek extends CI_Controller {
    public function index() {
      ...
      $req = request($this->input->post(),['botID','value'],['Podaj id bota!','Podaj czas do jakiego momentu ma być przesunięty utwór!']);
      if(!$req['success']) {
      ...
      foreach($list as $index) {
        if($index['Name'] == $req['botID']) {
          if($index['Status'] == 2) {
            ...
            $player = json_decode($this->ts3ab->seek($req['value']),true);
