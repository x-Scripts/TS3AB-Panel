<?php
  ...
  */
  ...
  class Repeat extends CI_Controller {
    public function index() {
      ...
      $req = request($this->input->post(),['botID','value'],['Podaj id bota!','Podaj czy ma zapętlać utwór/y utwory!']);
      if(!$req['success']) {
      ...
      foreach($list as $index) {
        if($index['Name'] == $req['botID']) {
          if($index['Status'] == 2) {
            ...
            $player = json_decode($this->ts3ab->repeat($req['value']),true);
